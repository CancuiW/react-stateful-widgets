...
export default function Programmers() {
  const [pro,setPro]=useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    //   if (pro[i].id===feature){
    //     return listOfAwesome[i].name
    //   }
    // }
    //第二种方法
    const found=pro.find(x=>x.id===feature)
    return found.name
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: feature ? "gold":'royalblue'// 🤔 color turns to gold, when celebrating
  };
